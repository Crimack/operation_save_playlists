...
def fix_playlist_tracks(client, tracks, blacklist):
    updated_tracks = []
    for track in tracks:
        logging.info(f'Fixing track: {track.name}, {track.artist}, {track.album}')
        ...
        exact_candidate_tracks = get_candidate_tracks(client, f'{track.name} {track.artist} {track.album}')
        ...
        chosen_fuzzy_candidate = prompt_for_input(track, fuzzy_candidate_tracks)
...
def transform_and_update_playlist(client, username, playlist, tracks):
    ...
    fixed_tracks = fix_playlist_tracks(client, normalised_tracks,
                                       blacklist)
    update_playlist(client, username, playlist, list(map(lambda t: t.uri, fixed_tracks)))
...
def main():
    ...
    playlists = filter(lambda y: y.name not in success, sorted(read_playlists(), key=lambda p: p.length))
